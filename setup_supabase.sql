...
-- Auto-profile on signup (ONLY for email signups, NOT Google/OAuth)
-- This trigger fires on INSERT (immediate signup) and UPDATE (email confirmation).
-- It creates a profile row ONLY for email/password signups that provided a username.
-- Google/OAuth users skip this trigger — the frontend onboarding page handles them.
...
DECLARE
  ...
  counter INTEGER := 0;
BEGIN
  -- GUARD 1: Only proceed if the email is confirmed.
  -- For auto-confirm ON: email_confirmed_at is set immediately on INSERT.
  -- For auto-confirm OFF: email_confirmed_at is set on UPDATE after user clicks link.
  IF NEW.email_confirmed_at IS NULL THEN
    RETURN NEW;
  END IF;

  -- GUARD 2: Don't create a duplicate profile
  IF EXISTS (SELECT 1 FROM public.profiles WHERE id = NEW.id) THEN
    RETURN NEW;
  END IF;

  -- GUARD 3: Only auto-create profile if 'username' was explicitly provided during signup.
  -- Email/password signups set username in user_metadata.
  -- Google/OAuth users will NOT have 'username' in metadata → skip them.
...
CREATE TRIGGER on_auth_user_created AFTER INSERT OR UPDATE ON auth.users FOR EACH ROW EXECUTE PROCEDURE handle_new_user();
